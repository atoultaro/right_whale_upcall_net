...
#config.EPOCHS = 10 # for testing, using smaller epoch number
##################################
# TEST PARAMETERS
...
#for rr in range(3):    
    ...
    while(best_accu < config.BEST_ACCU_HIST or np.isnan(best_accu)):
        ...
        if config.DO_AUGMENT == False:
            model.fit(X_train, Y_train, batch_size=config.BATCH_SIZE, 
                      ...
                      #epochs=config.EPOCHS, verbose=1, validation_data=(X_train, Y_train), callbacks=callbacks_list,
